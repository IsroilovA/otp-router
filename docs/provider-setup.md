...
Set Meta's `message_send_ttl_seconds` on the approved authentication template to a short window no greater than the challenge lifetime. Check existing templates explicitly: their defaults depend on when they were created. The adapter does not manage remote templates, and a template-level TTL cannot track the shorter remaining lifetime on resend. See [protocol constraints and sources](provider-research.md#meta-whatsapp-cloud-api) for supported values.
