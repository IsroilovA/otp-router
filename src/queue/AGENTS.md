...
- `QueueLive` owns startup and shutdown. Build it only after router migrations. Leave pg-boss migrations enabled.
...
- `Queue` exposes the internal pg-boss client for integration work. Feature code must use typed queue operations as they are implemented; do not scatter raw Promise calls or lifecycle calls through features.
